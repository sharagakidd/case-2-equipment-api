...
Данные хранятся в памяти процесса (без БД) — доступ к ним идёт только через слой репозиториев,
поэтому замена хранилища (например, на PostgreSQL) не затрагивает сервисы и контроллеры.
...
├── package-lock.json
├── README.md
├── data/                     # заготовка под файловое хранилище (.gitkeep)
└── src/
